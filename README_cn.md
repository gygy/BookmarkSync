...
修复：同一链接出现在多个文件夹时，合并会正确克隆，不再出现「计划 N、未写入 M」导致本地与云端条数差
新增：快速开始可导入配置备份 JSON，免去重新填写 Token
改进：向导内连通性测试结果改为醒目的成功/失败横幅
```
...
开发者模式或离线安装时，可从本仓库 [dist/](dist/) 获取最新安装包（当前 v1.1.43）：
