...
- **GitHub Gist** — 用 Personal Token 即可，上手简单  
- **私有 Gitea** — 自建 Git 仓库  
...
可以只用扩展、只用 Mac 应用，或两者同时用 — 指向**同一个** Gist / Gitea / WebDAV，即可跨浏览器保持一致。
...
1. 从浏览器扩展商店安装 **BookmarkSync**：  
   - [Chrome 网上应用店](https://chrome.google.com/webstore)（Edge 也可安装 Chrome 扩展）  
...
2. 进入 **连接** 标签。  
3. 选择存储类型：**GitHub Gist**、**Gitea** 或 **WebDAV**。  
4. 填写对应字段（见下文）。  
5. 点击 **测试连通性**。  
...
BookmarkSync 不运营同步服务器。书签与凭据仅保存在本机或你配置的 GitHub / Gitea / WebDAV 中。

完整说明：[隐私政策](docs/PRIVACY.md)
